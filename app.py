...
def register_user():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute("CALL register_user(%s, %s, %s, %s)", (name, email, password, address))
            conn.commit()
...
def profile():
    ...
    # Get user details
    cur.execute("SELECT name, email, address, role FROM users WHERE user_id = %s", (user_id,))
    user = cur.fetchone()
...
def update_cart(cart_item_id):
    ...
    cur.execute("UPDATE cart_items SET quantity = %s WHERE cart_item_id = %s", (quantity, cart_item_id))
    conn.commit()
    cur.close()
    conn.close()
